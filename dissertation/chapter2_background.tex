...
structure~\cite{long2015fully}. The encoder, typically a deep convolutional
...
The reproduced framework uses ResNet-101~\cite{he2016deep} as its
encoder backbone.
...
(ASPP)~\cite{chen2018encoder} addresses this by running multiple
...
deep features alone. U-Net~\cite{ronneberger2015u} addresses this
...
(BN)~\cite{ioffe2015batch}, which normalises layer activations across
...
problem. In the segmentation context, the model
...
segmentation~\cite{lee2013pseudo}. The model generates predictions
...
bias~\cite{arazo2020pseudo}. When the model's initial performance is
...
The Mean Teacher framework~\cite{tarvainen2017mean} helps reduce
...
DepthMix~\cite{hoyer2023improving} resolves this by using the predicted
...
(SSIM)~\cite{wang2004image} term. Because the entire supervision signal comes from the images themselves,
...
Within the framework of Hoyer et al.~\cite{hoyer2023improving}, SDE
...
pre-training~\cite{hoyer2023improving}, which penalises the $\ell_2$
...
Hoyer et al.~\cite{hoyer2023improving} demonstrate several ways in which
...
based on PAD-Net~\cite{xu2018pad}. PAD-Net maintains two parallel decoder
...
Zamir et al.~\cite{zamir2020robust} formalise it through inference-path
...
Nakano et al.~\cite{nakano2021cross} apply this principle directly to
multi-task learning by constraining intermediate features across task
branches, demonstrating improved performance. Li et
al.~\cite{li2022learning} study multi-task dense prediction under partial

...
At a high level, the proposed loss aligns intermediate features from the
depth and segmentation decoders through a lightweight projection module,
while leaving the original depth objective unchanged. Two similarity
formulations are considered, and the resulting design is evaluated with
respect to three questions: whether it improves segmentation performance
over the reproduced Stage~(b) MTL baseline, whether any improvement
remains consistent across different random seeds, and whether a stable
operating range for $\lambda_\text{ct}$ can be identified through
...
multi-task architecture. Two loss formulations (normalised MSE and cosine similarity) and two
projection directions---single~$(d{\rightarrow}s)$, which projects depth
features into the segmentation space, and single~$(s{\rightarrow}d)$,
which projects segmentation features into the depth space---are
investigated. A loss-scale calibration procedure followed by a
$\lambda$ sweep and a projection direction ablation are used to identify
the best design choices, and three independent random seeds are used to
verify whether the mIoU improvements over the Stage~(b) MTL baseline

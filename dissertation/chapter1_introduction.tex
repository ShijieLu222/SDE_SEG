...
To reduce this dependence on dense annotations, Hoyer et al.~\cite{hoyer2023improving}
...
training setup: a limited subset of images is treated as annotated and
used for supervised training with standard data augmentation. No SDE
components are involved at this stage, and it serves as the reference
point for measuring the improvements introduced in later stages.
...
which provide the basis for the architectural extension studied later.
Stage~(c) is outside the scope of this work because it addresses
cross-domain transfer and would require additional datasets,
domain-specific pipelines, and alignment strategies beyond the main focus
here. Instead, this project concentrates on improving feature interaction
within the same domain, where the relationship between depth and
segmentation representations can be examined more directly.
...
Hoyer et al.~\cite{hoyer2023improving} showed that SDE can provide a stable
geometric signal throughout the training pipeline, enabling
geometry-consistent data mixing and better feature transfer.

However, a structural limitation remains in the PAD-style multi-task
architecture adopted by Hoyer et al.~\cite{hoyer2023improving}, which is
based on attention-guided feature exchange between the depth and
segmentation decoders. While this architecture enables implicit cross-task
interaction, it places no direct constraint on the representations
themselves. More importantly, the depth decoder receives self-supervised
training signals from all available unlabelled frames throughout training,
while the segmentation decoder is only supervised on the small labelled
subset. This supervision asymmetry means the depth branch accumulates
richer geometric representations, yet there is no explicit mechanism to
transfer that geometric knowledge into the segmentation feature space.

This gap motivates the main extension proposed in this project: a
\textbf{Cross-Task Consistency Loss} ($\mathcal{L}_\text{ct}$) that
explicitly encourages the intermediate features of the depth and
segmentation decoders to remain aligned. Since pixels in the same semantic
region tend to share consistent depth values and sharp depth changes
typically coincide with semantic boundaries, features from the two decoders
should reflect compatible structural information about the same scene.
Rather than relying on the implicit feature exchange that the architecture
already provides, $\mathcal{L}_\text{ct}$ adds a direct alignment objective
so that the depth decoder can act as a continuously available, label-free
source of geometric supervision for the segmentation branch, precisely when
labelled data is most limited.

% -----------------------------------------------------------------------------
\section{Aims and Objectives}

This project has two main goals: to reproduce the SDE-enhanced segmentation
framework proposed by Hoyer et al.~\cite{hoyer2023improving}, and to extend
...
supervised baseline using a shared ResNet101~\cite{he2016deep} encoder
with an ASPP~\cite{chen2018encoder} decoder and
U-Net~\cite{ronneberger2015u} upsampling blocks, which serves as the
...
Loss.} Propose and implement $\mathcal{L}_\text{ct}$ within the PAD-style
multi-task architecture, investigating different loss formulations and
projection configurations. A systematic calibration procedure is used to
identify stable and well-performing design choices, with results validated
across multiple independent random seeds to confirm that improvements over
the Stage~(b) MTL baseline are reproducible.

\item \textbf{Evaluate the full combined framework.} Integrate
$\mathcal{L}_\text{ct}$ with the existing framework components---data
selection and DepthMix---to assess how well the proposed loss composes with
them and to measure the overall performance of the complete system.

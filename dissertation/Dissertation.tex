...
\begin{itemize}
  ...
  \item The four result tables of \cite{hoyer2023improving} (data
  ...
  \item The best-performing individual configuration in the PAD-MTL setting
    (single-projection cosine loss, $\lambda_\text{ct}=1.00$) yields a
    consistent $+2.50$\,pp improvement over the PAD-MTL baseline across three
    independent seeds, with a standard deviation of only $0.17$\,\%.
  
  \item When composed with the full framework, the strongest variant within
    the selected $d{\rightarrow}s$, $\lambda_\text{ct}=1.00$ design family
    (the MSE formulation) achieves $68.19$\,\% mIoU at the 372-label regime,
    exceeding both the reproduced full-framework baseline by $+0.53$\,pp
    and the original paper's multi-GPU result by $+0.18$\,pp.
...
label-efficient segmentation framework of \cite{hoyer2023improving} under
...
\item \textbf{Faithful reproduction of four result tables.}
  ...
  \cite{hoyer2023improving} were reproduced on Cityscapes. The data
...
\noindent
Several limitations remain, and they suggest natural directions for
future work.

\paragraph{Multi-GPU reproduction under the original training setting.}
A central limitation of this project is the constrained single-GPU
environment. All experiments were conducted with batch size~2 and without
Synchronised Batch Normalisation, whereas the original framework of
\cite{hoyer2023improving} was trained under a larger multi-GPU setting.
This difference is a plausible source of the reproduction gaps observed
in several configurations, especially those involving pseudo-label
training and the combination of DepthMix with multi-task learning. Future
work should repeat the main reproduction and $\mathcal{L}_\text{ct}$
experiments under a multi-GPU setup matching the original configuration
more closely, for example with an effective batch size of~8 and SyncBN
enabled. This would help separate the effect of the proposed loss from
hardware-induced optimisation differences and provide a fairer comparison
with the original paper.

\paragraph{Component-conditioned $\lambda$ scheduling.}
The composability ablation showed that the preferred loss type is
component-dependent. Within the selected $d{\rightarrow}s$ track, MSE
achieves higher gains when DepthMix is absent (MTL\,+\,S:
$+0.60$\,pp), while cosine is stronger when DepthMix is present
(MTL\,+\,DX: $+0.97$\,pp). The MSE variant ultimately achieves the
highest raw mean on MTL\,+\,DX\,+\,S ($68.19$\,\%), but with higher
variance (std~$1.03$\,\%) than the cosine counterpart. Treating
$\lambda_\text{ct}$ as a component-conditioned hyperparameter, or
annealing it when DepthMix is active, could reduce this variance without
sacrificing the mean-level gain.

\paragraph{Evaluation across additional label regimes and datasets.}
The proposed $\mathcal{L}_\text{ct}$ was primarily evaluated on
Cityscapes at the 1/8-label regime, where the relationship between
scene geometry and semantic layout is particularly strong. Future work
should test whether the same alignment objective remains effective under
more extreme or less constrained label budgets, such as the 1/30 and
1/4 regimes, and whether it generalises beyond urban driving scenes.
Datasets such as ADE20K~\cite{zhou2019semantic}, PASCAL VOC, or indoor
RGB-D benchmarks would provide useful tests of domain generality,
provided that suitable self-supervised or pretrained depth estimates are
available. A longer-term direction is to extend the same feature-alignment
idea to additional geometrically related auxiliary tasks, such as surface
normal estimation or optical flow.

\noindent
Overall, the results support the central hypothesis of this project:
explicit feature-level alignment between the depth and segmentation
decoder branches can improve label-efficient semantic segmentation under
a constrained single-GPU setting. While further validation is needed
under the original multi-GPU training configuration and beyond Cityscapes
at the 372-label regime, the proposed $\mathcal{L}_\text{ct}$ provides a
lightweight and reproducible extension to the SDE-enhanced PAD-MTL
framework.
...
\begin{itemize}
    ...
    mIoU, with the selected configuration highlighted.''}

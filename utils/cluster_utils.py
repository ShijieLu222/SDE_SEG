...
# from ray.tune.config_parser import create_trial_from_spec  # Commented out - deprecated in Ray 2.x
from ray.tune.search import BasicVariantGenerator  # Updated for Ray 2.x
from ray.tune.search.variant_generator import generate_variants, format_vars  # Updated for Ray 2.x
from ray.tune.search.variant_generator import _flatten_resolved_vars as flatten_resolved_vars  # Private API in Ray 2.x


class CustomVariantGenerator(BasicVariantGenerator):
    def __init__(self, shuffle=False):
        # Ray 2.x doesn't support 'shuffle' parameter, using default initialization
        super(CustomVariantGenerator, self).__init__()
        # Initialize counter for trial IDs
        self._counter = 0
        self._parser = None  # Not used in current implementation
    ...
    # Commented out - uses deprecated create_trial_from_spec API from Ray 0.x
